...
def processing(name):
    ...
    global MAX_DIAG_MULTIPLYER 
    MAX_DIAG_MULTIPLYER = 5 # 5
    global MAX_ANGLE_DIFF
    MAX_ANGLE_DIFF= 12.0 # 12.0
    global MAX_AREA_DIFF
    MAX_AREA_DIFF = 0.5 # 0.5
    global MAX_WIDTH_DIFF
    MAX_WIDTH_DIFF = 0.8
    global MAX_HEIGHT_DIFF
    MAX_HEIGHT_DIFF = 0.2
    global MIN_N_MATCHED
    MIN_N_MATCHED = 3 # 3
...
def test3(fname):
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        has_digit = False
        for c in chars:
            if ord('가') <= ord(c) <= ord('힣') or c.isdigit() or ord('A') <= ord(c) <= ord('Z') or ord('a') <= ord(c) <= ord('z'):
                if c.isdigit():
                    has_digit = True
                result_chars += c
